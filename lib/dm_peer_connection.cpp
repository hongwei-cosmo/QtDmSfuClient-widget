...
static std::unique_ptr<rtc::Thread> g_worker_network_thread;
...
bool DMPeerConnection::InitializePeerConnection(const char** turn_urls,
  ...
  if (g_peer_connection_factory == nullptr) {
    g_worker_network_thread = rtc::Thread::CreateWithSocketServer();
    g_worker_network_thread->SetName("worker_and_network_thread", nullptr);
    g_worker_network_thread->Start();
    g_signaling_thread = rtc::Thread::Create();
    g_signaling_thread->SetName("signaling_thread", nullptr);
    g_signaling_thread->Start();

    g_peer_connection_factory = webrtc::CreatePeerConnectionFactory(
        g_worker_network_thread.get(), g_signaling_thread.get(), nullptr,
    ...
    if (!peerConnectionFactoryOptions.crypto_options.enable_gcm_crypto_suites)
    {
      peerConnectionFactoryOptions.crypto_options.enable_gcm_crypto_suites = true;
      g_peer_connection_factory->SetOptions(peerConnectionFactoryOptions);
    }
  }
...
bool DMPeerConnection::CreatePeerConnection(const char** turn_urls,
                                                const int no_of_urls,
                                                const char* username,
                                                const char* credential) {
  ...
  config.media_crypto = std::make_shared<ChecksumMediaCrypto>();
...
void DMPeerConnection::DeletePeerConnection() {
  g_peer_count--;
  ...
  if (g_peer_count == 0) {
    ...
    g_worker_network_thread.reset();
  }
}

bool DMPeerConnection::CreateOffer() {
...
bool DMPeerConnection::CreateAnswer() {
  if (!peer_connection_.get())
    return false;
...
void DMPeerConnection::OnSuccess(
...
void DMPeerConnection::OnFailure(const std::string& error) {
...
void DMPeerConnection::OnIceCandidate(
...
void DMPeerConnection::RegisterOnLocalI420FrameReady(
    I420FrameReady_callback callback) {
  if (local_video_observer_)
    local_video_observer_->SetVideoCallback(callback);
}

void DMPeerConnection::RegisterOnRemoteI420FrameReady(
    I420FrameReady_callback callback) {
  if (remote_video_observer_)
    remote_video_observer_->SetVideoCallback(callback);
}

void DMPeerConnection::RegisterOnLocalDataChannelReady(
    LocalDataChannelReady_callback callback) {
  OnLocalDataChannelReady = callback;
}

void DMPeerConnection::RegisterOnDataFromDataChannelReady(
    DataFromDataChannelReady_callback callback) {
  OnDataFromDataChannelReady = callback;
}

void DMPeerConnection::RegisterOnFailure(Failure_callback callback) {
  OnFailureMessage = callback;
}

void DMPeerConnection::RegisterOnAudioBusReady(
    AudioBusReady_callback callback) {
  OnAudioReady = callback;
}

void DMPeerConnection::RegisterOnLocalSdpReadytoSend(
    LocalSdpReadyToSend_callback callback) {
  OnLocalSdpReady = callback;
}

void DMPeerConnection::RegisterOnIceCandiateReadytoSend(
    IceCandidateReadyToSend_callback callback) {
  OnIceCandiateReady = callback;
}

bool DMPeerConnection::SetRemoteDescription(const char* type,
                                                const char* sdp) {
  if (!peer_connection_)
    return false;
...
bool DMPeerConnection::AddIceCandidate(const char* candidate,
...
void DMPeerConnection::SetAudioControl(bool is_mute, bool is_record) {
...
void DMPeerConnection::SetAudioControl() {
  if (!remote_stream_)
    return;
...
void DMPeerConnection::OnAddStream(
...
DMPeerConnection::OpenVideoCaptureDevice() {
...
bool DMPeerConnection::CreateDataChannel() {
...
void DMPeerConnection::CloseDataChannel() {
  if (data_channel_.get()) {
...
bool DMPeerConnection::SendDataViaDataChannel(const std::string& data) {
  if (!data_channel_.get()) {
...
void DMPeerConnection::OnDataChannel(
    rtc::scoped_refptr<webrtc::DataChannelInterface> channel) {
  channel->RegisterObserver(this);
}

void DMPeerConnection::OnStateChange() {
  if (data_channel_) {
...
void DMPeerConnection::OnMessage(const webrtc::DataBuffer& buffer) {
...
void DMPeerConnection::OnData(const void* audio_data,
                                  int bits_per_sample,
                                  int sample_rate,
...
std::vector<uint32_t> DMPeerConnection::GetRemoteAudioTrackSsrcs() {
